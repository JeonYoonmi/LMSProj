...
<div style="width: 100%;background:#fff;padding:2%;border-top: 2px solid #001353;
	...
	<div style="width: 100%; min-width: 1200px; max-width: 1575px; margin: 0 auto;">
		<div class="app-search dropdown d-none d-lg-block" style="width: 300px;">
		    <form method="get" action="/mail/imsiMailBox">
					...
					<fmt:formatDate var="mlYndt" value='${ mailVO.mlYndt }' pattern='yyyy-MM-dd' />
						<li>
							...
							<div class="email-content">
								<a href="/mail/write?mlCd=${ mailVO.mlCd }" class="email-subject">
									${ mailVO.mlTtl }
								</a>
		...
		<div style="text-align: center;">
		    <button type="button" class="btn btn-light" <c:if test='${ list.startPage lt 6 }'>disabled</c:if> onclick="location.href='/mail/imsiMailBox?show=${ map.show }&cond=${ map.cond }&keyword=${ map.keyword }&currentPage=${ list.startPage - 5 }'"><i class="uil-angle-double-left"></i></button>
		    <button type="button" class="btn btn-light" <c:if test='${ list.startPage == list.currentPage }'>disabled</c:if> onclick="location.href='/mail/imsiMailBox?show=${ map.show }&cond=${ map.cond }&keyword=${ map.keyword }&currentPage=${ list.currentPage - 1 }'"><i class="uil uil-angle-left"></i></button>
			    ...
			    <button type="button" class="btn btn-<c:if test="${ list.currentPage != pNo }">light</c:if><c:if test="${ list.currentPage == pNo }">primary</c:if>"
		    		 onclick="location.href='/mail/imsiMailBox?show=${ map.show }&cond=${ map.cond }&keyword=${ map.keyword }&currentPage=${ pNo }'">
			    	${ pNo }
		    	</button>
			</c:forEach>
		    <button type="button" class="btn btn-light" <c:if test='${ list.endPage == list.currentPage }'>disabled</c:if> onclick="location.href='/mail/imsiMailBox?show=${ map.show }&cond=${ map.cond }&keyword=${ map.keyword }&currentPage=${ list.currentPage + 1 }'"><i class="uil uil-angle-right"></i></button>
		    <button type="button" class="btn btn-light" <c:if test="${ list.endPage ge list.totalPages }">disabled</c:if> onclick="location.href='/mail/imsiMailBox?show=${ map.show }&cond=${ map.cond }&keyword=${ map.keyword }&currentPage=${ list.startPage + 5 }'"><i class="uil-angle-double-right"></i></button>
		</div>
	</div>
</div>
<!-- end row-->