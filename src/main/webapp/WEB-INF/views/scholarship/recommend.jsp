...
function disableFunc(){
   ...
   success:function(result){
      if(result>1){
         ...
         $('#check').empty();
         let str='<span>이미 추천된 학생은 중복 추천 불가능합니다. 승인여부가 미수여 상태인 학생을 클릭하면 장학금 추천이 취소됩니다.</span></span><br><span class="comment">*이미 학생 2명을 추천하였습니다.</span>'
//          $('#btn1').hide();
         $('#check').append(str);
         cnt = 2;
         }else if(result == 0){
         ...
         $('#check').empty();
         $('#btn1').show();
         let str='<span>이미 추천된 학생은 중복 추천 불가능합니다. 승인여부가 미수여 상태인 학생을 클릭하면 장학금 추천이 취소됩니다.</span><br><span class="comment">*현재 학생 2명을 추천할 수 있습니다.</span>'
         $('#check').append(str);
         cnt = 0;
         }else if(result ==1){
         $('#grid').css('opacity','1');
         $('#check').empty();
         $('#btn1').show();
         let str='<span>이미 추천된 학생은 중복 추천 불가능합니다. 승인여부가 미수여 상태인 학생을 클릭하면 장학금 추천이 취소됩니다.</span><br><span class="comment">*현재 학생 1명을 추천할 수 있습니다.</span>'
         $('#check').append(str);
         cnt = 1;
         }
      }
...
var gridList2 = function(){
   ...
   $.ajax({
      ...
      success:function(result){
     	...
     	var dataSet = grid2.getData();
		console.log("테스트");
		console.log(dataSet);
		$.each(dataSet, function(i1, v1){
			if(v1.sclhYn == "수여"){
				grid2.addCellClassName(v1.rowKey, 'sclhYn', "addFontColorBlue");
			}
		});
...
<div style="width : 55%; float:left;">
   <p style="float : left;"><i class="mdi mdi-record-circle" style="color: #001353;"></i>&ensp;추천가능학생목록</p>
...
<div style="width : 44%; float:right; margin-left: 1%;">
   <p style="float : left;"><i class="mdi mdi-record-circle" style="color: #001353;"></i>&ensp;추천학생내역</p>
