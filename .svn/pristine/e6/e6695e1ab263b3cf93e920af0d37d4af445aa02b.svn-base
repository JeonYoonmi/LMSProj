...
public class socket extends TextWebSocketHandler {
	...
	protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
		...
		}else if(msgs[1].equals("reg")) { //나만 빼고 다른 사람들에게 메시지를 전송한다.
			
			String sessionId = (String)session.getId(); //현재 세션의 id
			TextMessage tmsg = new TextMessage(msgs[0]); //보낸 메시지 내용
			//System.out.println("sessionId : " + sessionId);
			
			for(int i = 0; i < sessions.size(); i++) { //비교해서 나만 빼고 메시지 전송하기
				String oneOfIds = (String)sessions.get(i).getId();
				//System.out.println("oneOfIds : " + oneOfIds);
				if(!oneOfIds.equals(sessionId)) {
					sessions.get(i).sendMessage(tmsg);
				}
			}
		}
