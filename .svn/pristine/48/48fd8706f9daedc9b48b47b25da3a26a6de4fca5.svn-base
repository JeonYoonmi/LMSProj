...
public class ArticlePage<T> {
	...
	public ArticlePage(int total, int currentPage, int size, List<T> content) {
		...
		if(total == 0) {
			...
			// 시작 페이지를 구하는 공식!
			// 시작페이지 = 현재 페이지 / 페이지 크기 * 페이지 크기 + 1
			startPage = currentPage / 5 * 5 + 1;
			
			// 전체 페이지 % 페이지 크기 => 0일 때 보정
			if(currentPage % 5 == 0) {
				// 페이지 크기를 빼줌
				startPage -= 5;
			}
			...
			System.out.println("ArticlePage : " + this.toString());
			
			// 종료 페이지 번호 > 전체 페이지수 보다 클 때
