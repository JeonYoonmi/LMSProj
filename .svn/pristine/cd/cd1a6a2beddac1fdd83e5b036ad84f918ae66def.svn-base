...
window.onload = function(){
	//처음 roomAssignDiv 가리기
	var roomAssignDiv = document.querySelector('#roomAssignDiv');
	var lecaApplyDiv = document.querySelector('#lecaApplyDiv');
	
	roomAssignDiv.style.display = 'none';
	lecaApplyDiv.style.display = 'block';
	
 	var lecaCd = ${lecaCd};	//강의계획서 코드
 	var mgrCd = ${mgrCd};	//관리자
 	
	//이미 승인이나 반려되었는지 확인하는 ajax
	data = {"lecaCd" : lecaCd};
	$.ajax({
		url : '/approval/confirmYn',
		type : 'post',
	...
	$('#noBut').on('click', function(){
	 	var memNmText = $('#memNmText').val();	//교수
	 	var lacaNm = $('#lecaNmText').val();	//과목명
	 	mgrCd = ${mgrCd};	//관리자
	 	lecaCd = ${lecaCd};	//강의계획서 코드
	 	$('#msgto').val(memNmText);
	 	$('#mailsubject').val(lacaNm);
	 	
	 	$('#modalsaveBtn').on('click', function(){
	 		var message = $('#simplemde1').val();
	...
	$('#okBut').on('click', function(){
		if(confirm('강의실 배정으로 넘어가시겠습니까?')){
			...
			roomAssignDiv.style.display = 'block';
			lecaApplyDiv.style.display = 'none';
		}else{
			alert("다시 처리해주세요");
			return false;
		}
	...
	//강의실 배정 script 시작
	//랜덤 색상 배열
	var colorArr = ['#ffebee', '#f3e5f5', '#e8eaf6', '#e3f2fd', '#e0f7fa', '#e8f5e9', '#fffde7', '#fff3e0', '#fbe9e7', '#e1f5fe', '#e0f2f1', '#fff8e1', '#fce4ec'];
...
<div class="bodyWrapper divWhole" id="lecaApplyDiv">
	...
	<table class="mb-0 topTable" border="1">
		<tr>
			...
			<td>
				<input id="lecaNmText" type="text" class="inputTop" disabled value="${data.lecaNm }">
			</td>
		...
		<tr>
			...
			<td>
				<input id="memNmText" type="text" class="inputTop" disabled value="${data.memberVO.memNm }">
			</td>
	...
	<div style="margin-top: 30px; margin-bottom : 20px;">
		<button class="btn btn-primary" id="okBut" style="margin-left: 500px; width:100px;">승인</button>
		<button class="btn btn-primary" id="noBut" data-toggle="modal" data-target="#compose-modal" style="width:100px;">반려</button>
	</div>
</div>
</div>
